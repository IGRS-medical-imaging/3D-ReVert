...
import dataset_svr.processor_pointcloud as pointcloud_processor
...
class ShapeNet(data.Dataset):
    ...
    def __init__(self, opt, train=True):
        ...
        if not opt.demo:
            ...
            # Load taxonomy file
            self.taxonomy_path = 'D:/SETUP/3DAttriFlow/taxonomy.json'
            ...
            # Select classes
            if opt.shapenet13 and not opt.class_choice:
            ...
            self.path_dataset = join(self.path_dataset,
                         self.opt.normalization + str(train) + "_" + "_".join(sorted(self.opt.class_choice)))
                ...
                if len(list_pointcloud) != 0:
                    ...
                    for pointcloud in list_pointcloud:
                        ...
                        if not self.opt.SVR or exists(image_path):
                            ...
                            red_print(f"Rendering not found for: {image_path}")
    ...
    def preprocess(self):
        if exists(self.path_dataset + "info.pkl"):
            ...
            with open(self.path_dataset + "info.pkl", "wb") as fp:  # Pickling
                pickle.dump(self.data_metadata, fp)
            torch.save(self.data_points, self.path_dataset + "points.pth")
        print(f"Total items in self.data_metadata: {len(self.data_metadata)}")
        print(f"Total items in self.datapath: {len(self.datapath)}")
        red_print("Dataset Size: " + str(len(self.data_metadata)))
...
if __name__ == '__main__':
    ...
    opt = EasyDict({
        "number_points": 2500,  # Number of points to sample
        "demo": False,  # Set to True to enable demo mode
        "shapenet13": True,  # If True, loads ShapeNet13 categories
        "class_choice": ["airplane"],  # Specific object classes to use
        "pointcloud_path": "D:/ShapeNetV1PointCloud/ShapeNetV1PointCloud",  # Path to point cloud files
        "image_path": "D:/ShapeNetV1Renderings/ShapeNetV1Renderings",  # Path to images
        "cache_path": "D:/ShapeNetcache_train/cache",  # Path to store cache
        "cache_path_test": "D:/ShapeNetcache_test/cache",
        "normalization": "UnitBall",  # Normalization method
        "SVR": True,  # Enable Single View Reconstruction
        "sample": True
    })
