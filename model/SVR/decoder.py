...
from utils.utils_model import position_encoding
cudnn.benchnark=True
from model.SVR.modules import *
...
class SP_DecoderEigen3steps(nn.Module):
    def __init__(self, args):
        ...
        # self.EdgeConv4 = EdgeBlock(dim[3], dim[4], self.nk )
        # self.adain4 = AdaptivePointNorm(dim[4], dim[-1])

        # self.EigenBlock1 = EigenBlock(num_points=4096, in_channels=32, n_basis=18)
        # self.EigenBlock2 = EigenBlock(num_points=4096, in_channels=64, n_basis=18)
        # self.EigenBlock3 = EigenBlock(num_points=4096, in_channels=128, n_basis=18)
        

        self.lrelu1 = nn.LeakyReLU(neg_2)
        self.lrelu2 = nn.LeakyReLU(neg_2)
        self.lrelu3 = nn.LeakyReLU(neg_2)
        self.lrelu4 = nn.LeakyReLU(neg_2)
